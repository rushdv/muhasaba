 ...
 * automatically on first run. We only manage our app tables here.
 *
 * Better Auth user IDs are TEXT (UUID), so foreign keys use TEXT.
 */
export const initDatabase = async () => {
  try {
    // muhasaba_logs — references Better Auth's "user" table
    await query(`
    ...
    // ramadan_reports — references Better Auth's "user" table
    await query(`
    ...
    console.error("❌ Error initializing app tables:", error);
    throw error;
  }
};
